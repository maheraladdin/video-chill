...
import { GreenEye, GreenUserCheck, GreenHeart } from "~/Components/Icons";
      ...
      <Layout closeSidebar={true}>
        <>
          {!data ? (
            ...
            <div className="flex flex-col gap-8 bg-white pt-3 sm:rounded-lg">
              ...
              <div>
                <dl className="mt-5 grid grid-cols-1 divide-y divide-gray-200 overflow-hidden rounded-2xl border border-gray-200  shadow-sm   md:grid-cols-3 md:divide-x md:divide-y-0">
                  {stats.map((item) => (
                    <div key={item.name} className="px-4 py-5 sm:p-6 flex flex-col gap-1">
                      {item.icon("h-16 w-16")}
                      <dt className="text-base font-normal text-gray-900">
                        {item.name}
                      </dt>
                      <dd className="text-3xl font-semibold text-primary-600 md:block lg:flex">
                        {item.stat}
                      </dd>
                    </div>
                  ))}
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                          {data?.videos.map((video) => (
                            <tr key={video.id}>
                              ...
                              <td className="whitespace-nowrap px-3 py-5 text-sm text-gray-500 flex gap-2">
